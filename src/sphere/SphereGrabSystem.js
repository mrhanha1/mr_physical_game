...
export class SphereGrabSystem {
  ...
  update(dt, frame) {
    ...
    for (const source of session.inputSources) {
      ...
      const gripObj = this.renderer.xr.getControllerGrip(hand === 'left' ? 0 : 1)
  ...
  _grabSphere(hand, sphereData, gripObj) {
    ...
    body.setLinearVelocity(this._zeroVec3, true)
    body.setAngularVelocity(this._zeroVec3, true)
  ...
  _releaseSphere(hand, throwVelocity) {
    ...
    if (speed > THROW_THRESHOLD) {
      ...
      body.setLinearVelocity({ x: v.x, y: v.y, z: v.z }, true)
