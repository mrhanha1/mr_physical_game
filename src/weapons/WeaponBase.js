...
export class WeaponBase {
  ...
  _fire(hapticHands = ['right']) {
    ...
    sphere.body.setLinearVelocity({ x: v.x, y: v.y, z: v.z }, true)

    // Xoay theo hướng bay
    const randomSpin = { x: (Math.random() - 0.5) * 20, y: (Math.random() - 0.5) * 20, z: (Math.random() - 0.5) * 20 }
    sphere.body.setAngularVelocity(randomSpin, true)
  ...
  _loadSphere(sphereData) {
    ...
    sphereData.body.setLinearVelocity({ x: 0, y: 0, z: 0 }, true)
    sphereData.body.setAngularVelocity({ x: 0, y: 0, z: 0 }, true)
